...
for i in range(0,len(x[0])):
	...
	try:
		...
		if ((nfailure+nsuccess)*(total_failed))!=0:
			fail_score=float(65*(nfailure-0.5)*(nfailure-0.5))/float((nfailure+nsuccess)*(total_failed))
...
csvfile=open(str_cwd+"/Fossum.csv", "a+")
