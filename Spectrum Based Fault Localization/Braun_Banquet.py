...
for i in range(0,len(x[0])):
	...
	try:
		a = nfailure+nsuccess
		b = total_failed
		if (0.5*(a*b))+(nsuccess*(total_failed-nfailure))!=0:
			fail_score=float(nfailure)/float((0.5*(a*b))+(nsuccess*(total_failed-nfailure)))
...
csvfile=open(str_cwd+"/Braun_Banquet.csv", "a+")
