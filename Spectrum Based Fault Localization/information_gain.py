...
for i in range(0,len(x[0])):
    ...
    try:
        ...
        if (ef == 0):
            ...
            t1 = float(ef)/float(ef + ep)
            h3 = t1*math.log10(t1)
        if (ep == 0):
            h4 = 0
        else:
            t2 = float(ep)/float(ep + ef)
            h4 = t2*math.log10(t2)
        if (nf == 0):
            h5 = 0
        else:
            t3 = float(nf)/float(nf + np1)
            h5 = t3*math.log10(t3)
        if (np1 == 0):
            h6 = 0
        else:
            t4 = float(np1)/float(nf + np1)
            h6 = t4*math.log10(t4)

        sus_score = -h1 - h2 - (ef+ep)*(-h3 - h4) - (nf+np1)*(-h5 - h6)
...
csvfile=open(str_cwd+"/information_gain.csv", "a+")
