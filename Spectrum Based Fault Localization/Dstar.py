...
for i in range(0,len(x[0])):
	...
	try:
		...
		if (total_failed-nfailure+nsuccess)!=0:
			fail_score=float(nfailure*nfailure)/float(total_failed-nfailure+nsuccess)

		sus_score=fail_score/(fail_score+pass_score)
		suspicious.append(sus_score)
		print(str(i)+"   "+str(sus_score))
	except ZeroDivisionError:
		if (total_failed == nfailure and nsuccess == 0):
            suspicious.append(9999)
        else:
            suspicious.append(0)
