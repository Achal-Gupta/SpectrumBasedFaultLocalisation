...
for i in range(0,len(x[0])):
	...
	for j in range(0,len(y)):
		...
		if x[j][i]==1 and y[j][0]==0:
			...
			nfailure=nfailure+1
	try:
		a = math.sqrt(float(nfailure*(total_passed-nsuccess)))
		#print nfailure,nsuccess
		if (a+total_failed+nsuccess)!=0:
			fail_score=float(a+nfailure)/float(a+total_failed+nsuccess)
...
csvfile=open(str_cwd+"/Baroni.csv", "a+")
