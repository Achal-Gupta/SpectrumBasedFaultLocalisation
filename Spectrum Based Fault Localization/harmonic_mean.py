...
for i in range(0,len(x[0])):
	...
	try:
		...
		# (total_passed- nsuccess)
		# (total_failed- nfailure)
		numerator=(nfailure*(total_passed- nsuccess)-(total_failed- nfailure)*nsuccess)*(((total_passed- nsuccess)+nsuccess)*(total_passed- nsuccess+total_failed-nfailure)+(nsuccess+total_failed-nfailure)*(nsuccess+total_passed- nsuccess))
		denominator=(nfailure+nsuccess)*(total_passed- nsuccess+ total_failed- nfailure)*total_failed*total_passed
		if denominator!=0:
			sus_score=float(numerator)/float(denominator)
...
csvfile=open(str_cwd+"/harmonic_mean.csv", "a+")
