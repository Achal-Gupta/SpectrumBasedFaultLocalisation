...
for i in range(0,len(x[0])):
    ...
    try:
        ...
        cbi_inc = float(ef)/float(ef + ep) - float(f)/float(f + p)
        if (cbi_inc == 0 or f == 0):
            sus_score = 0
        elif (ef == 0):
            sus_score = 2*cbi_inc
        else:
            sus_score = 1/float(1/cbi_inc + math.log10(f)/math.log10(ef))

        suspicious.append(sus_score)
        print(str(i)+"   "+str(sus_score))
    except ZeroDivisionError:
        suspicious.append(0) #Only possible if ef = ep = 0, since t!=0
...
csvfile=open(str_cwd+"/cbi_log.csv", "a+")
